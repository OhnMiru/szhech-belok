...
function saveCustomOrder() {
    ...
    document.querySelectorAll('#cards-container .card').forEach(card => {
        ...
        if (name) {
            const originalCard = originalCardsData.find(c => c.name === name);
            if (originalCard) order.push(originalCardsData.findIndex(c => c.name === name));
        }
    });
...
function loadCustomOrder() {
    ...
    if (saved) try { customOrder = JSON.parse(saved); } catch(e) { customOrder = []; }
    else customOrder = [];
}
...
function applyCustomOrder(data) {
    ...
    const ordered = [];
    for (const idx of customOrder) if (data[idx]) ordered.push(data[idx]);
    for (const item of data) if (!ordered.includes(item)) ordered.push(item);
    return ordered;
}
...
function dropHandler(e, dropIndex) {
    ...
    if (dragStartIndex !== null && dragStartIndex !== dropIndex) {
        ...
        currentFilteredData.forEach(card => {
            ...
            if (originalIndex !== -1) newOrder.push(originalIndex);
        });
