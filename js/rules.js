...
function addStructuredRule() {
    ...
    if (currentRuleType === 'type') {
        ...
        newRule = { type: "product", condition: { productNames, minQty }, message, icon: "🏷️", active: true };
    } else if (currentRuleType === 'price') {
...
function checkRulesForCart() {
    ...
    const typeQty = {};
    for (const [name, info] of Object.entries(cartInfo)) if (info.type) typeQty[info.type] = (typeQty[info.type] || 0) + info.qty;
    const activeRules = [];
    for (const rule of rulesList) {
        ...
        else if (rule.type === 'product') { let totalQty = 0; for (const productName of rule.condition.productNames) totalQty += cartInfo[productName]?.qty || 0; if (totalQty >= rule.condition.minQty) { isActive = true; detailText = `(в корзине ${totalQty} шт товаров из списка, нужно от ${rule.condition.minQty})`; } }
